...
- **Validação de regra SSH pela CLI:** campos como `*` podem ser retornados como valor único, lista ou `null` complementar pela API. Confirme no Portal a intenção da regra — prioridade `1000`, entrada TCP, origem somente no CIDR autorizado, destino `Any` e porta `22` — em vez de rejeitar uma regra correta pela forma de exibição.
